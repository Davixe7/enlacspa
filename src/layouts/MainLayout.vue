  ...
  <q-layout view="lHh Lpr lFf">
    <q-header>
      <q-toolbar style="padding: 36px 32px">
        <router-link to="/home">
          <q-img src="/logo_header.png" width="72px" />
        </router-link>
        ...
        <q-btn flat round icon="sym_o_rule" class="q-ml-md" :to="{ name: 'attendance.index' }"
          v-if="route.name == 'scores.create'" />

        <q-btn flat round icon="sym_o_notifications" class="q-mx-md" to="/notificaciones">
          <q-badge v-if="notificationStore.unreadNotificationsCount" color="red" floating rounded>
            {{ notificationStore.unreadNotificationsCount }}
          </q-badge>
        </q-btn>

        <q-avatar v-if="authStore.data.user" size="52px" text-color="white" color="grey" class="q-mr-sm">
