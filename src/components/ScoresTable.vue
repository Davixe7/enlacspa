...
<template>
  <q-table bordered flat dense :loading="loading" :columns="columns" :rows="rows" :pagination="{ rowsPerPage: 0 }"
    class="q-mb-md activity-candidates-table" style="width: 100%" no-data-label="NO HAY RESULTADOS PARA MOSTRAR">
    <template v-slot:body-cell-name="props">
      <q-td>
        <div class="multi-line-ellipsis">
          {{ props.value }}
          <q-tooltip anchor="top middle" self="bottom middle" max-width="300px" class="bg-dark text-white shadow-4">
            {{ props.value }}
          </q-tooltip>
        </div>
      </q-td>
    ...
    <template v-slot:body-cell-qualify="props">
      <q-td>
        <q-input v-if="props.row.activity.goal_type != 'Dominio'" :disable="readonly || props.row.closed == 1"
          type="number" borderless filled v-model="rows[props.rowIndex].score" style="max-width: 125px"
          :error="errors && !!errors[`scores.${props.rowIndex}.score`]" :error-message="'Complete el campo'">
        ...
        <q-select v-else :disable="readonly || props.row.closed == 1" dense outlined :options="[
        ...
        ]" v-model="rows[props.rowIndex].score" option-value="value" option-label="label" emit-value map-options
          use-chips style="max-width: 125px">
